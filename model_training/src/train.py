...
def make_dataprep_model(rent_data, test_size=0.3, random_state=42):
    ...
    print(f"column names of Training Data:{list(X_train.columns)}")
    print(f"column names of Test Data:{list(X_test.columns)}")
...
def scale_data(X_train, X_test, y_train, y_test):
    ...
    print(type(y_test))
    ## saving these standard scaler info for predict function as per https://stackoverflow.com/questions/38780302/predicting-new-data-using-sklearn-after-standardizing-the-training-data
    std  = np.sqrt(sc_X.var_)
    np.save('std_train.npy',std )
    np.save('mean_train.npy',sc_X.mean_)
    ## scaler object is saved as per https://stackoverflow.com/questions/41993565/save-minmaxscaler-model-in-sklearn
    pickle.dump(sc_X, open(scalerfile, 'wb')) ## saving the scaler object
    pickle.dump(sc_y, open(result_slacerfile, 'wb')) ## saving the scaler object
...
def model_voting_regressor(X_train, X_test, y_train, y_test, best_KNN_estimator, best_dtc_estimator, best_rfc_estimator, random_state=100):
    ...
    df_voting_test = pd.DataFrame(voting_test,columns = ['Predicted_Rent'])
    df_voting_test.to_csv("df_voting_test.csv",index=False)

    df_org_y_test = pd.DataFrame(y_test,columns = ['Actual_Rent'])
    df_org_y_test.to_csv("df_org_y_test.csv",index=False)
...
def run():
    with mlflow.start_run() as run:
        ...
        rent_data = read_input_file()
        rent_data = preprocess_record(rent_data)
        X, y, X_train, X_test, y_train, y_test = make_dataprep_model(rent_data, test_size=0.3, random_state=42)
        X_train, X_test, y_train, y_test = scale_data(X_train, X_test, y_train, y_test)
        ## doing LR
        lm, lm_prediction, metrics = model_lr(X_train, X_test, y_train, y_test)
        save_model(lm, "linear_regression")
        mlflow.log_param("LR metrices", metrics)
        ## doing decision tree
        dt, dt_prediction,metrics = model_dt(X_train, X_test, y_train, y_test)
        save_model(dt, "decision_tree")
        mlflow.log_param("decision_tree metrices", metrics)
        ## doing decision tree regressor
        best_dtc_estimator, mse_dtr, metrics = model_dt_regressor(X_train, X_test, y_train, y_test, random_state=100)
        save_model(best_dtc_estimator, "decision_tree_regressor")
        mlflow.log_param("decision_tree_regressor metrices", metrics)
        ## doing svm
        svr, svr_prediction, metrics = model_svm(X_train, X_test, y_train, y_test)
        save_model(svr, "svm")
        mlflow.log_param("svm metrices", metrics)
        ## doing random forerst
        rf, rf_prediction, metrics = model_rf(X_train, X_test, y_train, y_test, n_estimators=100)
        mlflow.log_param("random_forest metrices", metrics)
        save_model(rf, "random_forest")
        ## doing random forest regressor
        best_rfr_estimator, drfc_prediction, metrics = model_rf_regressor(X_train, X_test, y_train, y_test)
        save_model(best_rfr_estimator, "random_forest_regressor")
        mlflow.log_param("random_forest_regressor metrices", metrics)
        ## knn regressor
        best_KNN_estimator, KNN_test, metrics = model_knn_regressor(X_train, X_test, y_train, y_test)
        save_model(best_KNN_estimator, "knn_regressor")
        mlflow.log_param("knn_regressor metrices", metrics)
        ## voting regressor
        model_voting, voting_test, metrics = model_voting_regressor(X_train, X_test, y_train, y_test, best_KNN_estimator, best_dtc_estimator, best_rfr_estimator,)
